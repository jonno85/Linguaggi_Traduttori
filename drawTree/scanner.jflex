...
obj = (file|cartella|cartelle|files|directory|directories|dir)
...
file = ({id})+"."({id})+
file_wwc = ({sp_char}|{id})+("."({sp_char}|{id}))+
...
{com_script_start}			{
					exclude_fs=false; 
					System.out.println("232378dentro script");
					//yybegin(script);
...
{int}" "?"kb"				{return symbol(sym.IUnit);}
{float}" "?"kb"			{return symbol(sym.FUnit);}
...
{file_wwc}					{System.out.println("file con wildcards: "+yytext());
					return symbol(sym.File_wwc,new String(yytext()));}
